...
# --- UPDATED: UNIVERSAL PDF TO ANY CONVERTER WITH FILTERS ---
async def pdf_to_any_logic(file: UploadFile, target_ext: str, background_tasks: BackgroundTasks):
    ...
    # Define filters to prevent "no export filter" errors
    # LibreOffice needs these hints to convert PDF back to editable formats
    filters = {
    ...
    try:
        # We use 'writer_pdf_import' as the in-filter so LibreOffice 
        # treats the PDF as text/data rather than a static image/drawing.
        cmd = [
            LO_BINARY, 
            "--headless", 
        ...
        files = [f for f in os.listdir(out_dir) if f.lower().endswith(f".{target_ext}")]
        if not files: 
            # Fallback check if the extension was slightly different
            files = os.listdir(out_dir)
            if not files: raise Exception("No output file generated by LibreOffice")
        
        out_path = os.path.join(out_dir, files[0])
        background_tasks.add_task(cleanup, in_path)
        background_tasks.add_task(cleanup, out_dir)
        
        return FileResponse(out_path, filename=f"converted_result.{target_ext}")
...
# These routes remain the same, just calling the improved logic above
